...
export class EcosystemService {
  ...
  public static async hashPassword(password: string): Promise<string> {
    // Fallback para contextos não seguros (HTTP sem localhost/127.0.0.1) onde crypto.subtle não está disponível
    ...
    try {
      ...
      // Fallback em caso de erro na execução nativa
      return this.sha256Fallback(password);
    }
  }

  /**
   * Fallback puro JS de SHA-256 para contextos não seguros (HTTP)
   */
  private static sha256Fallback(ascii: string): string {
    ...
    for (let i = 7; i >= 0; i--) {
      asciiBytes.push((asciiBitLength >>> (i * 8)) & 0xff);
    }
