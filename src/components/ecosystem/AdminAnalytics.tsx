...
export function AdminAnalytics() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      {/* Gráfico de Evolução */}
      <Card className="lg:col-span-2 border border-white/10 shadow-2xl bg-slate-900/40 backdrop-blur-xl text-white">
        <CardHeader>
          <CardTitle className="text-xl font-black uppercase tracking-wider flex items-center gap-2 text-slate-100">
            <TrendingUp className="h-5 w-5 text-indigo-400" />
            Evolução Semanal
          </CardTitle>
          <CardDescription className="text-slate-400">Volume de resíduos coletados nos últimos 7 dias (kg).</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[300px] w-full min-h-[300px]">
            {isMounted ? (
              ...
              <div className="h-full w-full bg-slate-950 animate-pulse rounded-xl" />
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-6">
        ...
        <Card className="border-none bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 hover:border-emerald-500/35 shadow-[0_0_30px_rgba(16,185,129,0.05)] text-white overflow-hidden relative rounded-[2rem]">
          <Globe className="absolute -bottom-4 -right-4 h-24 w-24 text-emerald-500 opacity-10 rotate-12" />
          <CardHeader className="pb-2">
            <CardTitle className="text-lg font-black uppercase tracking-wider flex items-center gap-2 text-emerald-400">
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="text-3xl font-black text-white">{co2Saved} kg</div>
              <p className="text-xs text-emerald-400">CO2 evitado na atmosfera</p>
            </div>
            <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-2xl backdrop-blur-sm">
              ...
              <div>
                <div className="font-bold text-white">{treesSaved} Árvores</div>
                <p className="text-[10px] text-emerald-400 font-medium">Equivalência em preservação</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Ranking de Turmas */}
        <Card className="border border-white/10 shadow-2xl bg-slate-900/40 backdrop-blur-xl text-white rounded-[2rem]">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg font-black uppercase tracking-wider flex items-center gap-2 text-slate-100">
              ...
              {turmaRanking.map((turma, index) => (
                <div key={turma.name} className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    index === 0 
                      ? "bg-yellow-400/10 text-yellow-400 border-yellow-400/20 shadow-[0_0_10px_rgba(250,204,21,0.2)]" 
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs font-bold text-slate-200">{turma.name}</span>
                      <span className="text-xs font-black text-indigo-400">{turma.points} pts</span>
                    </div>
      ...
      <div className="lg:col-span-3 grid gap-6 md:grid-cols-3">
        <Card className="border border-white/10 shadow-2xl bg-slate-900/40 backdrop-blur-xl text-white">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-black uppercase tracking-wider flex items-center gap-2 text-slate-200">
              <div className="h-2.5 w-2.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)] animate-pulse" />
              Artigos Lidos
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-white">{pedagogicStats.articles}</div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Total de leituras na unidade</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-white/10 shadow-2xl bg-slate-900/40 backdrop-blur-xl text-white">
          ...
          <CardContent>
            <div className="text-3xl font-black text-white">{pedagogicStats.quizzes}</div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Avaliações concluídas</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-white/10 shadow-2xl bg-slate-900/40 backdrop-blur-xl text-white">
          ...
          <CardContent>
            <div className="text-3xl font-black text-white">{pedagogicStats.rewards}</div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Trocas de Bio-Coins efetuadas</p>
          </CardContent>
        </Card>
      </div>
    </div>
