...
export function WasteChart() {
  ...
  return (
    <Card className="border border-slate-200/60 dark:border-white/10 shadow-2xl bg-white/80 dark:bg-slate-900/40 backdrop-blur-xl text-slate-800 dark:text-white overflow-hidden relative group rounded-[2rem]">
      <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none text-slate-400 dark:text-white">
        ...
        <ChartContainer config={chartConfig} className="w-full" height={350}>
          <PieChart>
            <Pie
              ...
              paddingAngle={5}
              strokeWidth={2}
              stroke={theme === 'dark' ? "#070913" : "#ffffff"}
              animationBegin={200}
