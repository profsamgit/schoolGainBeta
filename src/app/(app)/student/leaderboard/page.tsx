    ...
    return (
      ...
      >
        <div className={cn(
          ...
          <div className="bg-white/95 dark:bg-slate-950/90 border border-slate-200 dark:border-transparent backdrop-blur-3xl rounded-[2.2rem] p-6 md:p-8 text-center w-48 md:w-64 space-y-5 relative overflow-hidden">
            <div className={cn(
            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="relative mb-4">
                    <div className={cn(
                        "w-20 h-20 md:w-24 md:h-24 rounded-full border-2 p-1 bg-slate-50 dark:bg-white/5 overflow-hidden shadow-2xl",
                        isFirst ? "border-emerald-400/50" : "border-slate-200 dark:border-white/10"
                    )}>
                        <div className={cn(
                    ...
                    <div className={cn(
                        "absolute -bottom-1 -right-1 w-10 h-10 rounded-full flex items-center justify-center font-black text-sm shadow-xl border-2 border-slate-200 dark:border-slate-950",
                ...
                <div className="space-y-1.5 w-full">
                    <h3 className="text-[14px] md:text-lg font-black text-slate-800 dark:text-white px-2 uppercase tracking-tight text-center leading-tight line-clamp-2 min-h-[2.5rem] flex items-center justify-center">
                        {formatDisplayName(user.name)}
                    </h3>
                    <div className="flex items-center justify-center gap-1.5">
                        <Flame className={cn("h-4 w-4", isFirst ? "text-emerald-500 dark:text-emerald-400" : "text-slate-400 dark:text-white/40")} />
                        <span className={cn(
                          "text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]",
                          user.level === 'Guardião da Lenda' ? "text-yellow-600 dark:text-yellow-400 animate-pulse" : "text-slate-500 dark:text-white/50"
                        )}>{user.level}</span>
                    </div>
                </div>

                <div className="pt-5 border-t border-slate-200 dark:border-white/10 w-full mt-5">
                    <p className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white leading-none tabular-nums tracking-tighter">
                        {user.totalScore.toLocaleString()}
                    </p>
                    <p className="text-[9px] font-black uppercase text-emerald-600 dark:text-emerald-400/80 tracking-[0.25em] mt-2">Score XP</p>
                </div>
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pb-20">
      ...
      <div className="relative pt-20 pb-16 flex flex-col items-center">
         <div className="absolute top-0 left-1/2 -translate-x-1/2 -z-10 opacity-15">
             <Trophy className="h-64 w-64 text-emerald-500 rotate-12" />
         </div>
         
         <div className="text-center space-y-4 mb-20 relative z-10">
            <h2 className="text-5xl md:text-7xl font-black text-slate-800 dark:text-white tracking-tighter uppercase flex items-center justify-center gap-6">
               <Star className="text-emerald-500 dark:text-emerald-400 animate-pulse h-10 w-10" fill="currentColor" />
               Pódio da Elite
               <Star className="text-emerald-500 dark:text-emerald-400 animate-pulse h-10 w-10" fill="currentColor" />
            </h2>
            <p className="text-slate-500 dark:text-slate-400 font-black text-base uppercase tracking-[0.3em]">Os Guardiões da Biosfera em Destaque</p>
         </div>
      ...
      <div className="px-6">
        <div className="max-w-5xl mx-auto bg-white/85 dark:bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-12 shadow-lg dark:shadow-2xl relative overflow-hidden border border-slate-200 dark:border-white/5 transition-all duration-300">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="space-y-4 text-center md:text-left">
                 <div className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-400/10 rounded-full border border-yellow-400/20">
                    ...
                    <span className="text-[10px] font-black text-yellow-600 dark:text-yellow-400 uppercase tracking-widest">Exclusividade Mensal</span>
                 </div>
                 <h3 className="text-4xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Hall das Lendas</h3>
                 <p className="text-slate-500 dark:text-slate-400 font-bold text-sm max-w-md uppercase tracking-wide leading-relaxed">
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center gap-4">
                         <div className={cn(
                            ...
                            legend ? "bg-emerald-500/25 border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)] animate-pulse" : "bg-slate-100 dark:bg-white/5 border-dashed border-slate-200 dark:border-white/10"
                          )}>
                            {legend ? (
                               legend.avatar ? (
                                  <img src={legend.avatar} alt={legend.name} className="w-full h-full object-cover" />
                               ) : (
                                  <span className="text-3xl font-black text-slate-800 dark:text-white">{legend.name.charAt(0)}</span>
                               )
                            ) : (
                               <Trophy className="h-8 w-8 text-slate-300 dark:text-white/10" />
                            )}
                         </div>
                         <div className="text-center">
                            <p className={cn(
                               "text-[10px] font-black uppercase tracking-widest",
                               legend ? "text-emerald-650 dark:text-emerald-400" : "text-slate-400 dark:text-slate-650"
                            )}>
      ...
      <Card className="border border-slate-200 dark:border-white/5 shadow-lg dark:shadow-2xl bg-white/85 dark:bg-slate-900/40 rounded-[2rem] overflow-hidden text-slate-800 dark:text-white backdrop-blur-xl transition-all duration-300">
        <CardHeader className="border-b border-slate-250 dark:border-white/5 p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1">
              <CardTitle className="flex items-center gap-2 text-2xl font-black uppercase tracking-tight text-slate-805 dark:text-white">
                <Flame className="h-6 w-6 text-orange-500 dark:text-orange-400 animate-pulse" />
                Corrida Sustentável
              </CardTitle>
              <CardDescription className="font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[9px]">
            ...
            <Tabs value={rankingRole} onValueChange={(val: any) => setRankingRole(val)} className="w-full md:w-auto">
              <TabsList className="bg-slate-100 dark:bg-slate-950/60 p-1 h-12 rounded-full border border-slate-200 dark:border-white/5">
                <TabsTrigger value="student" className="rounded-full px-8 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400 data-[state=active]:bg-emerald-500 data-[state=active]:text-white transition-all duration-300">
                   Alunos
                </TabsTrigger>
                <TabsTrigger value="staff" className="rounded-full px-8 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400 data-[state=active]:bg-emerald-500 data-[state=active]:text-white transition-all duration-300">
                   Equipe
                </TabsTrigger>
              </TabsList>
            </Tabs>
 
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-full px-4 py-2 text-slate-600 dark:text-slate-350 shadow-inner">
                <Trophy className="h-4 w-4 text-emerald-555 dark:text-emerald-400" />
          ...
          <Table>
            <TableHeader className="bg-slate-100 dark:bg-slate-950/40">
              <TableRow className="border-b border-slate-200 dark:border-white/5">
                <TableHead className="w-24 text-center h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Pos</TableHead>
                <TableHead className="h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Agente</TableHead>
                <TableHead className="hidden md:table-cell h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Nível</TableHead>
                <TableHead className="text-center h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Vitalidade</TableHead>
                <TableHead className="text-right h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Score Global</TableHead>
                <TableHead className="w-24 text-center h-14 font-black uppercase text-[9px] tracking-widest text-slate-500 dark:text-slate-400">Visita</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {restUsers.map((user, index) => (
                <TableRow
                  key={user.id}
                  className={cn(
                    "group transition-all duration-300 border-b border-slate-200 dark:border-white/5 hover:bg-slate-100/50 dark:hover:bg-white/5",
                    user.ra === currentUser?.ra && "bg-emerald-500/10 hover:bg-emerald-500/15 border-l-[6px] border-l-emerald-500 text-slate-800 dark:text-white",
                    user.level === 'Guardião da Lenda' && "bg-amber-500/10 hover:bg-amber-500/15 border-l-[6px] border-l-amber-500 shadow-[inset_0_0_20px_rgba(245,158,11,0.05)] text-slate-850 dark:text-white"
                  )}
                >
                  <TableCell className="font-black text-center text-slate-450 dark:text-slate-400 text-xl py-6 tabular-nums group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors border-b border-slate-200 dark:border-white/5">
                    {index + 4}º
                  </TableCell>
                  <TableCell className="border-b border-slate-200 dark:border-white/5">
                    <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-[1rem] bg-slate-100 dark:bg-slate-950 flex items-center justify-center font-black text-slate-500 dark:text-slate-400 shadow-sm border border-slate-200 dark:border-white/5 group-hover:border-emerald-500/30 transition-colors overflow-hidden">
                            {user.avatar ? (
                                <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                            ) : (
                                user.name.charAt(0)
                        ...
                        <div>
                            <span className={cn(
                                ...
                                user.ra === currentUser?.ra ? "font-black text-emerald-600 dark:text-emerald-400" : "font-bold text-slate-800 dark:text-slate-200"
                            )}>
                                {formatDisplayName(user.name)}
                                ...
                                {index + 4 === 1 && <Leaf className="ml-2 h-4 w-4 text-yellow-500 dark:text-yellow-400 inline" fill="currentColor" />}
                            </span>
                            <span className="text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest md:hidden">{user.level}</span>
                        </div>
                    </div>
                  </TableCell>
                  <TableCell className="hidden md:table-cell border-b border-slate-200 dark:border-white/5">
                    ...
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="flex items-center gap-2">
                          <span className={cn(
                            ...
                            user.displayVitality >= 70 ? 'text-emerald-650 dark:text-emerald-400' : 'text-amber-650 dark:text-amber-400'
                          )}>
                            {user.displayVitality}%
                          </span>
                      </div>
                      <div className="w-24 h-1.5 bg-slate-250 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-250 dark:border-white/5">
                        <div
                          className={cn(
                  ...
                  <TableCell className="text-right border-b border-slate-200 dark:border-white/5">
                    <div className="flex flex-col items-end gap-0.5">
                        <span className="font-black text-2xl text-slate-800 dark:text-white tabular-nums tracking-tighter">
                            {user.totalScore.toLocaleString()}
                        </span>
                        <span className="text-[7px] font-black text-slate-500 dark:text-slate-450 uppercase tracking-widest">Pontos XP</span>
                    </div>
                  </TableCell>
