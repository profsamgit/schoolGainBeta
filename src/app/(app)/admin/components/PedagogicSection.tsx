    ...
    return (
      <Card className="border border-white/10 shadow-2xl overflow-hidden bg-slate-900/40 rounded-[2rem] backdrop-blur-xl text-white animate-in fade-in duration-300">
        <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 bg-slate-950/20 px-6 py-5">
          <div>
            <CardTitle className="text-lg font-black uppercase tracking-tight text-indigo-400">{isNew ? 'Novo Artigo' : 'Editar Artigo'}</CardTitle>
            <CardDescription className="text-slate-400 text-xs mt-1">Conteúdo educativo para os alunos.</CardDescription>
          </div>
        ...
        <CardContent className="p-8">
          <Form {...articleForm}><form onSubmit={articleForm.handleSubmit(onSubmit)} className="space-y-6">
            <FormField control={articleForm.control} name="title" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Título</FormLabel>
            ...
            <FormField control={articleForm.control} name="summary" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Resumo (aparece no card)</FormLabel>
            ...
            <FormField control={articleForm.control} name="content" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Conteúdo do Artigo</FormLabel>
                <FormControl><Textarea {...field} className="min-h-[200px] bg-slate-950 border-white/10 text-white rounded-xl focus:border-indigo-500/50 font-bold" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-slate-950 border border-white/5 rounded-2xl shadow-md">
              ...
              <FormField control={articleForm.control} name="imageHint" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Emoji ou Ícone de Destaque</FormLabel>
            ...
            <FormField control={articleForm.control} name="videoUrl" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">URL do Vídeo (Opcional)</FormLabel>
                <FormControl><Input {...field} placeholder="https://youtube.com/..." className="h-12 bg-slate-950 border-white/10 text-white rounded-xl focus:border-indigo-500/50 font-bold" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 space-y-2 mt-6 animate-pulse">
              ...
              />
              <p className="text-[9px] text-amber-500/60 font-medium italic">Autorização necessária para publicar alterações pedagógicas.</p>
            </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-none overflow-hidden relative bg-indigo-950/40 border border-indigo-500/20 text-white rounded-[2rem] shadow-2xl p-6 backdrop-blur-xl">
      ...
      <Card className="border border-white/10 shadow-2xl overflow-hidden bg-slate-900/40 rounded-[2rem] backdrop-blur-xl hover:border-indigo-500/10 transition-all duration-300 text-white">
        <CardHeader className="border-b border-white/5 bg-slate-950/20 px-6 py-5">
          <CardTitle className="text-sm font-black uppercase tracking-widest text-slate-200">Tópicos de Quiz</CardTitle>
          <CardDescription className="text-slate-400 text-xs mt-1">Geração automática de desafios para os alunos no ecossistema.</CardDescription>
        </CardHeader>
        <CardContent className="p-6 space-y-4">
          <div className="flex gap-2">
            <Input placeholder="Novo tópico (ex: Reciclagem, Consumo Consciente, Biodiversidade, Energias Renováveis, Desmatamento)" value={newTopic} onChange={(e) => setNewTopic(e.target.value)} className="h-12 bg-slate-950 border-white/10 text-white rounded-xl focus:border-indigo-500/50 font-bold" />
            <Button onClick={handleAddTopic} className="h-12 w-12 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-slate-950 border border-indigo-400/20 font-black rounded-xl shadow-lg transition-transform hover:scale-105"><Plus className="h-4 w-4" /></Button>
          </div>
          <div className="flex flex-wrap gap-2 pt-2">
            {[...filteredQuizTopics].sort((a, b) => a.name.localeCompare(b.name)).map((topic, idx) => (
              <Badge key={`${topic.id}-${idx}`} className="bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-bold uppercase text-[9px] px-3 py-1.5 rounded-xl hover:border-indigo-500/30 transition-all flex items-center gap-1.5 shadow-sm">
                {topic.name}
                <Button size="icon" variant="ghost" className="h-4 w-4 p-0 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-full" onClick={() => handleDeleteTopic(topic)}>
      ...
      <Card className="border border-white/10 shadow-2xl overflow-hidden bg-slate-900/40 rounded-[2rem] backdrop-blur-xl hover:border-indigo-500/10 transition-all duration-300 text-white">
        <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 bg-slate-950/20 px-6 py-5">
          <div>
            <CardTitle className="text-sm font-black uppercase tracking-widest text-slate-200">Biblioteca de Conteúdo</CardTitle>
            <CardDescription className="text-slate-400 text-xs mt-1">Artigos pedagógicos e links de apoio em vídeo.</CardDescription>
          </div>
        ...
        <CardContent className="p-6 space-y-6">
          <div className="p-4 bg-slate-950 border border-white/5 rounded-2xl shadow-md">
            <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 block ml-1">Pesquisar Artigo</Label>
            <Input placeholder="Buscar por título..." value={articleSearch} onChange={(e) => setArticleSearch(e.target.value)} className="h-12 bg-slate-950 border border-white/10 text-white rounded-xl focus:border-indigo-500/50 font-bold" />
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-950/50 overflow-hidden shadow-2xl">
            <Table>
              <TableHeader className="bg-slate-950 border-b border-white/10">
                <TableRow>
                  <TableHead className="font-black uppercase text-[10px] tracking-widest text-slate-400 px-6 h-12">Título</TableHead>
                    ...
                    <TableCell className="text-right px-6 py-4">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-slate-950 border border-white/10 text-white rounded-xl shadow-2xl p-1 min-w-[120px]">
                          <DropdownMenuItem onClick={() => handleEdit(article, 'article')} className="hover:bg-indigo-500/10 cursor-pointer font-bold text-xs uppercase tracking-wider py-2.5 rounded-lg"><Edit className="mr-2 h-4 w-4" />Editar</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleDelete(article, 'article')} className="text-rose-400 hover:bg-rose-500/10 cursor-pointer font-bold text-xs uppercase tracking-wider py-2.5 rounded-lg"><Trash2 className="mr-2 h-4 w-4" />Excluir</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
