...
function AdminContent() {
  ...
  // Captura de RFID por teclado global (USB emulator)
  useEffect(() => {
    ...
    const handleGlobalKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        if (buffer.length >= 4) {
          userForm.setValue('rfid', buffer.toUpperCase());
          setIsRFIDCapturing(false);
          toast({ title: "Cartão Capturado", description: `ID: ${buffer.toUpperCase()}` });
        }
        buffer = '';
  ...
  }, [isRFIDCapturing, userForm, toast]);
  ...
  useEffect(() => {
    ...
    const activeSchoolTerminals = terminals.filter(
      t => t.schoolId === targetSchoolId && t.status === 'active'
    );
      ...
      try {
        for (const terminal of activeSchoolTerminals) {
          ...
          if (res.ok) {
            ...
            if (data && data.ra) {
              userForm.setValue('rfid', data.ra.toUpperCase());
              setIsRFIDCapturing(false);
  ...
  }, [isRFIDCapturing, targetSchoolId, terminals, userForm, toast]);
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6 w-full">
          <div className="w-full overflow-x-auto no-scrollbar pb-1">
            <TabsList className="!inline-flex w-max min-w-full justify-start h-16 bg-slate-100/80 dark:bg-slate-950/80 p-1.5 rounded-2xl border border-slate-200/60 dark:border-white/5 shadow-2xl backdrop-blur-xl">
              ...
              <TabsTrigger value="academic" className="flex-1 shrink-0 min-w-max !inline-flex rounded-xl data-[state=active]:bg-indigo-600 dark:data-[state=active]:bg-indigo-500 data-[state=active]:text-white font-black uppercase text-[10px] tracking-widest transition-all duration-300 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><Users className="mr-2 h-4 w-4" /> Acadêmico</TabsTrigger>
              <TabsTrigger value="pedagogic" className="flex-1 shrink-0 min-w-max !inline-flex rounded-xl data-[state=active]:bg-indigo-600 dark:data-[state=active]:bg-indigo-500 data-[state=active]:text-white font-black uppercase text-[10px] tracking-widest transition-all duration-300 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><BookOpen className="mr-2 h-4 w-4" /> Pedagógico</TabsTrigger>
              <TabsTrigger value="economic" className="flex-1 shrink-0 min-w-max !inline-flex rounded-xl data-[state=active]:bg-indigo-600 dark:data-[state=active]:bg-indigo-500 data-[state=active]:text-white font-black uppercase text-[10px] tracking-widest transition-all duration-300 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><Gift className="mr-2 h-4 w-4" /> Econômico</TabsTrigger>
              <TabsTrigger value="infra" className="flex-1 shrink-0 min-w-max !inline-flex rounded-xl data-[state=active]:bg-indigo-600 dark:data-[state=active]:bg-indigo-500 data-[state=active]:text-white font-black uppercase text-[10px] tracking-widest transition-all duration-300 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><Cpu className="mr-2 h-4 w-4" /> Infra</TabsTrigger>
            </TabsList>
          </div>
