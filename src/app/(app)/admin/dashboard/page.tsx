...
export default function AdminDashboardPage() {
    const { users, currentUser, wasteEntries } = useEcosystem();
    
    // Filtra dados pela escola do gestor logado
    const isSuperAdmin = currentUser?.role === 'super_admin';
    const schoolId = currentUser?.schoolId;

    const filteredUsers = users.filter(u => 
        isSuperAdmin ? true : u.schoolId === schoolId
    );

    const filteredWasteEntries = wasteEntries.filter(entry => 
        isSuperAdmin ? true : entry.schoolId === schoolId
    );
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Total de Resíduos (Ciclo Atual)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {filteredWasteEntries.reduce((acc, curr) => acc + curr.collected, 0).toFixed(1)} kg
                        </div>
                        <p className="text-xs text-muted-foreground">Apenas coletas processadas neste ciclo</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
