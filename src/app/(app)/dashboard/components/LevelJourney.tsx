  ...
  return (
    <div className="w-full py-4 px-4 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-3">
        ...
        {nextLevel && (
            <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-white/20 dark:border-white/5 px-4 py-2 rounded-xl flex items-center gap-3 shadow-sm group transition-all hover:shadow-md">
            ...
            return (
              ...
              >
                ...
                <div className={cn(
                  ...
                  isReached 
                    ? cn("bg-white dark:bg-slate-900 shadow-xl scale-110", level.color, "border-current") 
                    : "bg-slate-100 dark:bg-slate-800 border-transparent text-slate-400 scale-90 grayscale opacity-50"
                )}>
                  {isCurrent && (
                    <div className={cn("absolute inset-0 rounded-2xl animate-ping opacity-20", level.bg)} />
                  )}
                  <Icon className={cn("w-6 h-6 md:w-8 md:h-8 transition-transform group-hover:scale-110", isReached ? "drop-shadow-sm" : "")} />
                  
                  {/* Badge de Pontos */}
                  <div className={cn(
                    "absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter transition-all duration-500",
                    isReached ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "bg-slate-200 text-slate-400 dark:bg-slate-800"
                  )}>
                ...
                <span className={cn(
                  ...
                  isReached ? "text-slate-900 dark:text-white" : "text-slate-400"
                )}>
                  {level.name}
                </span>
              </div>
      ...
      {/* LEGENDA MOBILE */}
      <div className="mt-8 md:hidden flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-800/50 py-3 rounded-2xl">
